...
author: "<span class='pres-title'>Sorting, Samples, Saving,<br/>and other R basics</span>"
output: 
  slidy_presentation:
...
NULL can not exist in a vector, but NA can. 
...
A List object is an ordered group of data that are not of the same type. Lists are similiar to vectors except that,
...
- instead of using c() like in vector creation, use <tt>list()</tt> to create a list
...
*.RData* and *.Rda* are common extensions for R data files, but you can name your file anyting you want. 

The __<tt>save()</tt>__ function saves objects in a binary, compressed file format that R understands. This makes it very flexible and easy to bring back into R using the sister function __<tt>load()</tt>__.
...
__Other ways to save data__

If you want to export data to use with another program, you probably want to use a different function that saves in a more standard file format, like __<tt>write.csv()</tt>__ or __<tt>write.table()</tt>__.

Use the __<tt>sink()</tt>__ function to redirect output from the console to a file. Run __<tt>sink()</tt>__ again without an argument to resume sending output to the console.

Spatial objects can be written to GIS formats with functions from the *rgdal* package, including __<tt>writeGDAL</tt>__ (vector) and __<tt>writeGDAL</tt>__ (raster). 

Plots can be saved as PNG and PDF files.
...
[Exercise 4. Lists and sorting](../exercises/ex04_sort-save-misc.html){target="_blank"}
