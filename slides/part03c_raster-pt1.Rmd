...
```{r viz_geotiff, fig.align="center", cache=TRUE}
...
library(kableExtra)
tbl_vec <- c("visualize", "image(), plot()", "crop", "crop()", "mask", "mask()", "resample (i.e., change the pixel size)", "aggregate()", "(re)project", "projectRaster()", "inspect pixel values", "getValues(), [rows,cols]", "stack", "stack()", "export to disk", "writeRaster(), writeGDAL()")
...
```{r get_data_yosem_srtm, cache=TRUE}
library(raster)
western_ca_dem_ll <- getData('SRTM', lon=-119, lat=37, download=TRUE)
...
```{r, echo=FALSE, results='asis'}
tbl_vec <- c("dimensions", "dim(), nrow(), ncol()", "extent", "extent(), x@extent",
